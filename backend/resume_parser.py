...
def retrieve_relevant_chunks(vector_store, query: str, k: int = 5) -> list[str]:
    ...
    """
    try:
        # Preferred LangChain API
        docs = vector_store.similarity_search(query, k=k)

    except Exception:
        try:
            # Fallback for some vectorstore wrappers
...
def inspect_vector_store(vector_store) -> dict:
    ...
    try:
        # Attempt to get an approximate document count
        if hasattr(vector_store, "_collection"):
            coll = getattr(vector_store, "_collection")
            # Chroma collection object sometimes exposes count()
            try:
...
# LOAD & RANK UTILITIES
# =========================================================
def load_vector_store(collection_name: str = "resume_chunks", persist_directory: str | None = None):
    """
    Load an existing Chroma vector store. Returns the vector store instance.
    """
    try:
        # Preferred constructor for persisted stores
        if persist_directory:
            return Chroma(collection_name=collection_name, persist_directory=persist_directory)
        # Fallback to in-memory with collection name
        return Chroma(collection_name=collection_name)
    except Exception:
        try:
            # Some LangChain versions use from_existing_collection or similar
...
def retrieve_relevant_resumes(
    ...
    """
    try:
        # Try to get docs with scores first
        try:
        ...
        for doc, score in results or []:
            ...
            if not resume_id:
                # skip chunks without resume linkage
                continue
            ...
            # accumulate semantic signal
            if score is None:
                entry["semantic_raw"] += 1.0
            else:
                try:
                    # lower distance -> better, so convert to similarity-ish
            ...
            # keep top snippets (dedupe by text)
            text = getattr(doc, "page_content", str(doc))
            if text not in entry["snippets"]:
                entry["snippets"].append(text)

        # Apply basic metadata filters and compute scores
        items = []
        ...
        for rid, v in agg.items():
            ...
            # Years score: cap at 1.0 by dividing by 10 (heuristic)
            try:
